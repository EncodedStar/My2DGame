...
---@field public TransformMode ESkyAtmosphereTransformMode @The ground albedo that will tint the astmophere when the sun light will bounce on it. Only taken into account when MultiScattering>0.0.
---@field public BottomRadius number @The planet radius. (kilometers from the center to the ground level).
---@field public GroundAlbedo FColor @The ground albedo that will tint the astmophere when the sun light will bounce on it. Only taken into account when MultiScattering>0.0.
---@field public AtmosphereHeight number @The planet radius. (kilometers from the center to the ground level).
...
---@field public AerialPerspectiveStartDepth number @The distance (kiloneters) at which we start evaluating the aerial pespective.       Keeping this value a little away from the camera can help with performance: pixels not affected by the aerial perspective will have their computation skipped using early depth test.
